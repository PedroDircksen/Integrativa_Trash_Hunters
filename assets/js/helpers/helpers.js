...
;(function($, Helpers){
	...
	var username = function(username){
		...
		username.keyup(function(){ usernameValue() })
		username.click(function(){ usernameValue() })
		username.focus(function(){ usernameValue() })
		username.blur(function(){ usernameValue() })
	...
	var pmfsMask = function(pmfs){

		var pmfs = pmfs || $('#pmfs-ibama');
		var mask  = '00000.000000/0000-00';
		pmfs.mask(mask);
	...
	var ftMask = function(weight){
		weight.maskMoney({ suffix: " ft", decimal:",", thousands:"."});
	};		
	var mMask = function(weight){
		weight.maskMoney({ suffix: " m", decimal:",", thousands:"."});
	};		
	var cmMask = function(weight){
		weight.maskMoney({ suffix: " cm", decimal:",", thousands:"."});
	};		
	var kgMask = function(weight){
		weight.maskMoney({ suffix: " kg", decimal:",", thousands:"."});
	};		
	var ptsMask = function(weight){
		weight.maskMoney({ suffix: " pts", decimal:",", thousands:"."});
	}
	var temperatureMask = function(temperature){
		temperature.maskMoney({ suffix: " ºC", decimal:",", thousands:"."});
	};		
	var landlineMask = function (phone) {
		phone.maskMoney({ prefix: "+", decimal:"", thousands:""});
	};
	var areaMask = function(area){
		area.maskMoney({ suffix: "ha", decimal:",", thousands:".", precision:4});
	};		
	...
	Helpers.pmfsMask		 	= pmfsMask;
    // Helpers.weightMask		 	= weightMask;
	Helpers.dapMask		 	= dapMask;
